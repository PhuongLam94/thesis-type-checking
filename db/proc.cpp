 ...
 * RETURNS:			tbc
 *============================================================================*/
void UserProc::typePropagation(std::map<Exp*, ConstantVariable*>& map){
    ...
    ExpTypeVisitor* v = new ExpTypeVisitor();
...
bool UserProc::unionCheck(std::list<UnionDefine*>& unionDefine, std::map<Exp*, ConstantVariable*> mapExp) {
        ...
        if (status < PROC_VISITED)
         ...
         bool valid = true;
                ...
                for (PBB bb = cfg->getFirstBB(it); bb; bb = cfg->getNextBB(it)) {
                    //bb->calcReachingDef();
                    if(!bb->checkUnion(unionDefine, replacement, mapExp)){
                        valid = false;
                    }
                   status = PROC_FINAL;
                }
        ...
        if (status < PROC_VISITED)
         ...
         BB_IT it;
                ...
                for (PBB bb = cfg->getFirstBB(it); bb; bb = cfg->getNextBB(it)) {
                   ...
                   if(!bb->replaceAcc2(unionDefine, mapExp, replacement)){
                        valid = false;
                   }
